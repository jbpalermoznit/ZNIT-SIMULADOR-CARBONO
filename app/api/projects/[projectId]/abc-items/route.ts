...
export async function GET(
  ...
  // An auto-exclusion is identifiable by mapped_by='auto_excluded' (new
  // rows after the subcontract-rescue fix) OR — for legacy rows written
  // by the old code — by the canonical justification text generated by
  // autoExclusionReason / laborOnlyExclusionReason.
  ...
  const results = allItems.map((item) => {
    ...
    const isAutoExcluded =
      ...
      (m?.mapped_by === "auto_excluded" ||
        autoExcludedMarkers.some((marker) => justification.includes(marker)));
    return {
      id: item.id,
